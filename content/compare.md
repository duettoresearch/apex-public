APEX overlaps with several established categories without sitting cleanly in any
of them. This page places it next to each, states what the comparator does that
APEX does not, and keeps every claim to something a reader can check.
...
**ADR tooling** is dramatically cheaper to adopt. `adr-tools` is a shell script.
...
**Docs-as-code sites** publish better. Docusaurus and MkDocs produce fast,
searchable, versioned documentation sites with navigation, versioned URLs, and
search out of the box. APEX's repository is a source of truth, not a reading
experience — the Companion app exists because reading raw markdown in a private
repository is a poor way to consume it. APEX retired its own Docusaurus site
rather than maintain two publishing surfaces.

**Shape Up** is a method, and methods travel where systems do not. Appetite,
betting tables, and fixed time with variable scope are ideas a team can adopt on
Monday with no tooling at all. APEX takes no position on how work is scoped or
bet on; an initiative's hypothesis field will hold a Shape Up pitch as readily as
anything else.
...
**Productboard, Aha!, and Jira Product Discovery** are better at the things a
database is better at: prioritization scoring across hundreds of items, customer
feedback ingestion at volume, roadmap views for people who will never open a
terminal, and integrations that already exist. APEX's aggregate views are
generated reports, and its prioritization is whatever a human writes in a field.
...
someone who has never used git. APEX's answer for non-technical contributors is
a companion web app, which is a narrower surface than a full wiki.

## Where APEX is different

Two things distinguish it, and they are the same thing seen from two angles.
...
artifacts, so a change to the contract and the migration of every artifact to
match it can arrive in one reviewable pull request — and until they do, CI is red.
...
`git revert`, and subject to the identical schema validation. That is less
...
The contract also has to be maintained. {{stats.artifactTypes}} artifact types is
a real surface, and a change to one propagates through the schema, the templates,
the skills, the handbook, and the validator. APEX's own specification tracks the
discrepancies where a change landed in one layer and not the others, which is an
honest way to record a cost that does not disappear.
